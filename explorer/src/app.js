...
app.use('/ext/getchartsdata', function(req,res) {
    const now = Date.now();
    const twoWeeksAgo = Math.floor(new Date(now - (now % 86400000) - 1209600000).valueOf() / 1000); // minus seconds to noon and minus week
    const blankResponse = {tx: {}, amount: {}};
    tx.find({
        timestamp: {
            $gt: twoWeeksAgo
        }
        ...
        for (let i = 0; i < data.length; i++) {
            const tx = data[i];
            const day = tx.timestamp - (tx.timestamp % 86400);
            aggregatedAmount[day] = (aggregatedAmount[day] || 0) + (tx.total / 100000000);
            aggregatedTxs[day] = (aggregatedTxs[day] || 0) + 1;
        }
